...
def test_rawg_select_avoids_demo_when_query_not_dlc_like() -> None:
    ...
    best, _score, _top = RAWGClient._select_best_candidate(query="Trine 2", candidates=cands, year_hint=2011)
    assert best is not None
    assert best["id"] == 2
...
def test_rawg_select_uses_year_hint_to_prefer_release_near_year() -> None:
    ...
    best, _score, _top = RAWGClient._select_best_candidate(query="Doom", candidates=cands, year_hint=1993)
    assert best is not None
    assert best["id"] == 2
