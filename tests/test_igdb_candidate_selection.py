...
def test_igdb_select_prefers_main_game_category_over_dlc() -> None:
    ...
    best, _score, _top = IGDBClient._select_best_match(
        query="Example", results=results, year_hint=2000
    )
...
def test_igdb_select_prefers_exact_title_over_numbered_variant() -> None:
    ...
    best, _score, _top = IGDBClient._select_best_match(
        query="Diablo", results=results, year_hint=None
    )
    assert best is not None
    assert best["id"] == 2
...
def test_igdb_select_uses_year_hint_to_prefer_release_near_year() -> None:
    ...
    best, _score, _top = IGDBClient._select_best_match(
        query="Doom", results=results, year_hint=1993
    )
    assert best is not None
    assert best["id"] == 2
